...
{
    Mapping res = new_kernel_mapping();
    ElfHeader *e_header = (ElfHeader *)elf;
    // 校验 ELF 头
    if (e_header->magic != ELF_MAGIC)
    {
        panic("Unknown file type!");
    }
    ProgHeader *p_header = (ProgHeader *)((usize)elf + e_header->phoff);
    // 遍历所有的程序段
    for (int i = 0; i < e_header->phnum; ++i, ++p_header)
    {
        ...
        Segment segment = {start_va, end_va, flags};
